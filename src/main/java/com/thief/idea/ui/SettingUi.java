...
public class SettingUi {
    ...
    /**
     * 伪装代码语言下拉：不在 .form 里声明，由构造函数运行时追加到 Style 面板（同 ttsKey 的做法）
     **/
    public JComboBox<String> codeLanguage;

    /**
     * 自定义模型名输入框（纯伪装，只填名字，不需要 url / api key）：
     * 同样不在 .form 里声明，由构造函数运行时追加到 Style 面板
     **/
    public JTextField customModel;

    /**
     * 单行 shell 代码块开关：不在 .form 里声明，由构造函数运行时追加到 Style 面板
     **/
    public JCheckBox shellBlock;

    /**
     * 无图模式开关（隐藏正文里的电子书插图）：同样运行时追加到 Style 面板
     **/
    public JCheckBox hideImages;
    ...
    public SettingUi() {
        ...
        // 伪装代码语言：与 TTS 热键同理，运行时给 Style 面板追加一行
        // （fontsPanel 的 .form 里有 13 行，追加两行后新控件落在第 15 行）
        codeLanguage = new JComboBox<>(DisguiseContent.LANGUAGES);
        codeLanguage.setToolTipText("Language of the diff cards shown in the assistant replies.");
        FormLayout styleLayout = (FormLayout) fontsPanel.getLayout();
        styleLayout.appendRow(new RowSpec("6dlu"));
        styleLayout.appendRow(new RowSpec("center:default:grow"));
        CellConstraints languageConstraints = new CellConstraints();
        fontsPanel.add(new JLabel("Code language:"), languageConstraints.xy(2, 15));
        fontsPanel.add(codeLanguage,
                languageConstraints.xy(4, 15, CellConstraints.FILL, CellConstraints.DEFAULT));

        // 自定义模型名：同样运行时追加一行（追加两行后 fontsPanel 已有 15 行，新控件落在第 17 行）。
        // 只收一个名字，纯装饰——界面上会假装这是"接入的自定义模型"，实际不发任何请求
        customModel = new JTextField();
        customModel.setToolTipText("Display-only custom model name shown in the model list. No endpoint or API key needed.");
        styleLayout.appendRow(new RowSpec("6dlu"));
        styleLayout.appendRow(new RowSpec("center:default:grow"));
        fontsPanel.add(new JLabel("Custom model:"),
                languageConstraints.xy(2, 17));
        fontsPanel.add(customModel,
                languageConstraints.xy(4, 17, CellConstraints.FILL, CellConstraints.DEFAULT));

        // 单行 shell 代码块开关：同样运行时追加一行（追加两行后 fontsPanel 已有 17 行，落在第 19 行）。
        // 勾选后会在较长的自然段中间插一条单行 shell 命令，把段落切开
        shellBlock = new JCheckBox("Shell snippet in paragraphs");
        shellBlock.setToolTipText("Insert a one-line shell command in the middle of a paragraph (disguise only).");
        styleLayout.appendRow(new RowSpec("6dlu"));
        styleLayout.appendRow(new RowSpec("center:default:grow"));
        fontsPanel.add(shellBlock, languageConstraints.xyw(2, 19, 3, CellConstraints.FILL,
                CellConstraints.DEFAULT));

        // 无图模式：隐藏正文里的电子书插图（追加两行后 fontsPanel 已有 19 行，落在第 21 行）。
        // 只影响渲染，不会去动 epub 解包出来的图片目录
        hideImages = new JCheckBox("Hide book images");
        hideImages.setToolTipText("No-image mode: skip illustrations when rendering the page (they are not needed for reading).");
        styleLayout.appendRow(new RowSpec("6dlu"));
        styleLayout.appendRow(new RowSpec("center:default:grow"));
        fontsPanel.add(hideImages, languageConstraints.xyw(2, 21, 3, CellConstraints.FILL,
                CellConstraints.DEFAULT));
    }
    ...
    public void innit(PersistentState persistentState) {
        ...
        codeLanguage.setSelectedItem(persistentState.getCodeLanguage());
        customModel.setText(persistentState.getCustomModel());
        shellBlock.setSelected(persistentState.isShellBlockEnabled());
        hideImages.setSelected(persistentState.isHideImages());
        updateFontPreview();
    }
    ...
    /**
     * 当前选择的伪装代码语言（下拉项即 DisguiseContent.LANGUAGES）
     **/
    public String getSelectedCodeLanguage() {
        Object selected = codeLanguage.getSelectedItem();
        return selected == null ? DisguiseContent.DEFAULT_LANGUAGE : selected.toString();
    }

    /**
     * 输入框里填的自定义模型名（纯伪装），未填返回空串
     **/
    public String getCustomModel() {
        String text = customModel.getText();
